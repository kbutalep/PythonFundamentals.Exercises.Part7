...
greetings_dict = {1: ['Hello', 'Hi', 'Howdy'], 2: ['Hola', 'ola', 'Hola mi amor'], 3: ['Bonjour', 'Salut', 'Bonsoir']}
...
def greet(name: str, greetings_options: Dict[int, str], lang_choice: int) -> None:
    ...
    :return:
    """
    greeting = str(random.choice(greetings_options[lang_choice]))
    print(greeting + " " + name)
...
def select_user_mode():
    ...
    if user_input == 1:
        ...
        print("That's not an option. Entering user mode.\n")
...
def admin_update_language():
    ...
    new_greeting = input()
    greetings_dict[lang_selection][2] = new_greeting
    ##greetings_dict[lang_selection][0].append(new_greeting) <---figure out how to append list
